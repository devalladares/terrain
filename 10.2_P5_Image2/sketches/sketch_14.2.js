...
export default function adaptedSketchN(p) {
  ...
  const palette = [ 
    ...
    "white",          // White
    '#96D39B',        // Light Green
    // '#57D7F2',      // Light Blue (Commented Out)
    // '#F098F4'        // Light Pink (Commented Out)
  ];
  ...
  p.setup = () => {
    ...
    // Calculate number of columns based on aspect ratio to maintain grid
    N_ROWS = N;
  ...
  p.draw = () => {
    ...
    for (let i = margin; i < N_COLS - margin; i++) {
      for (let j = margin; j < N_ROWS - margin; j++) {
        ...
        const dynamicCrossSize = Math.min(cellWidth, cellHeight) * 0.3; // Dynamic cross size
        // Horizontal arm
        p.line(
          centerX - dynamicCrossSize / 2,
          centerY,
          centerX + dynamicCrossSize / 2,
          centerY
        );
