export default function adaptedSketchN(p) {
  // Grid Configuration
  const N = 24;                    // Base number of cells (determines row count)
  const FRAME_RATE = 5;           // Animation speed in frames per second
  ...
  const LETTER_SIZE_FACTOR = 0.6;  // Letter size relative to cell size

  // Flow Field Configuration
  const flowScale = 0.5;           // Scale of the background flow field (larger = more variation)
  const letterFlowScale = 0.03;    // Scale of the letter flow field (smaller = more gradual changes)
  let zOffset = 0;                 // Z-offset for background flow field animation
  let letterZOffset = 0;           // Z-offset for letter flow field animation
  const zIncrement = 0.004;        // Speed of background flow field animation
