export default function(p) {
  const CONFIG = {
    ...
    PATH_LENGTH: 10000,
    RESET_INTERVAL: 3000,
    FADE_SPEED: 5 // Controls how quickly trails fade (0-1)
  };
  ...
  class Particle {
    ...
    draw() {
      ...
      for (let i = 0; i < this.path.length; i++) {
        ...
        // Calculate fade based on position in path
        const fadeProgress = i / this.path.length;
        // Use fade speed to adjust the fade curve
        const fadeMultiplier = Math.pow(fadeProgress, 1 - CONFIG.FADE_SPEED);
        const alpha = Math.floor(CONFIG.OPACITY_PERCENT * 2.55 * fadeMultiplier);
  ...
  function addGUIControls() {
    // Set initial GUI background color
    gui.domElement.style.backgroundColor = 'rgb(0, 45, 0)';
    ...
    pathFolder.add(CONFIG, 'FADE_SPEED', 0, 1).step(0.01)
      .name('Fade Speed')
      .onChange(resetSketch);
