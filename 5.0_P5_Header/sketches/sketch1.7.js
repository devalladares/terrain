...
export default function(p) {
  ...
  const CONFIG = {
    ...
    LINE_WEIGHT: 0.75,
    PARTICLE_LIFETIME_SEC: 10, // Displayed in seconds
  ...
  function initGUI() {
    ...
    // Line Weight
    gui.add(CONFIG, 'LINE_WEIGHT', 0.1, 5, 0.1).name('Line Weight').onChange(value => {
      // Adjust line thickness
    }).title = "Thickness of the particle lines.";

    // Stroke Alpha (New GUI Control)
    gui.add(CONFIG, 'STROKE_ALPHA', 0, 255, 1).name('Stroke Alpha').onChange(value => {
      // Adjust stroke transparency in Particle.draw()
    }).title = "Transparency of the particle lines (0-255).";

    // Particle Lifetime in Seconds
    gui.add(CONFIG, 'PARTICLE_LIFETIME_SEC', 1, 100, 1).name('Particle Lifetime (sec)').onChange(value => {
    ...
    gui.add(CONFIG, 'SHOW_BG_IMAGE').name('Show Background').onChange(value => {
      // Toggle background image visibility
    }).title = "Toggle the visibility of the background image.";

    // Background Opacity
    gui.add(CONFIG, 'BG_OPACITY', 0, 255).name('Background Opacity').onChange(value => {
      // Adjust background opacity
    }).title = "Opacity of the background image (0-255).";
