export default function(p) {
  const CONFIG = {
    ...
    COLORS: [
      // '#F098F4',   // Pink
      // '#57D7F2', // Blue (commented out)
      '#ffffff', // White (commented out)
      '#96D39B'    // Lightgreen
    ],
    USE_COLORS: true,  // Set to true to use colors array, false for white only
    LINE_WEIGHT: 2,    // Increased for better visibility in dashed lines
    TRAIL_LENGTH: 5000,  // Default trail length (can be overridden per particle)
    FLOW_VECTOR_SCALE: 3,
    ...
    TRAIL_LENGTH_RANGE: [1000, 1000],  // Range for trail length variation
    MOUSE_SPAWN_RADIUS: 40,          // Radius around cursor for spawning particles
    TRAIL_STYLE: 'circle',            // 'circle' or 'dashed'
    CIRCLE_SIZE: 2,                   // Diameter of trail circles
    CIRCLE_SPACING: 5.25,               // Minimum distance between circles
    ...
    particles.forEach(particle => {
      ...
      if (particle.trail.length > 1) {
        ...
        if (CONFIG.TRAIL_STYLE === 'circle') {
          // Draw filled circles at trail points with spacing
          p.noStroke(); // Remove stroke for filled circles
          ...
          // Draw dashed lines between trail points
          p.drawingContext.setLineDash([CONFIG.DASH_LENGTH, CONFIG.GAP_LENGTH]);
          p.stroke(particle.color); // Reapply stroke after setLineDash
          p.strokeWeight(CONFIG.LINE_WEIGHT);
  ...
  p.keyPressed = function() {
    ...
    if (p.key === 'T' || p.key === 't') {
      if (CONFIG.TRAIL_STYLE === 'circle') {
        CONFIG.TRAIL_STYLE = 'dashed';
        console.log('Trail style changed to Dashed Lines');
      } else {
