export default function(p) {
  const CONFIG = {
    ...
    COLOR_SCHEMES: {
      WHITE: ['#ffffff'],
      GREEN: ['#96D39B'],
      MIXED: ['#ffffff', '#96D39B', '#96D39B', '#96D39B']  // 3:1 ratio favoring green
    },
    ...
    CURRENT_TRAIL_STYLE: 'stroke',  // Starting trail style
    STROKE_WEIGHT: 0.2,              // Corrected from LINE_WEIGHT
    MIN_STROKE_WEIGHT: 0.2,          // Minimum stroke weight
    MAX_STROKE_WEIGHT: 10,           // Maximum stroke weight
    CIRCLE_SIZE: 2,                   // Removed duplicate
    PLUS_SIZE: 3,        // Size of plus symbols
    SQUARE_SIZE: 2,      // Size of squares
    TRAIL_LENGTH: 5000,
    FLOW_VECTOR_SCALE: 3,
    LINE_OPACITY: 255,
    ...
    CIRCLE_SPACING: 5.25, // Moved TRAIL_STYLE removed to prevent inconsistency
    DASH_LENGTH: 10,
    GAP_LENGTH: 20,
    USE_COLORS: true, // Added
    COLORS: ['#ffffff', '#96D39B'], // Added if needed
  };
  ...
  p.keyPressed = function() {
    ...
    // *** New Functionality: Adjust Stroke Weight ***

    // Increase strokeWeight with ']' key
    if (p.key === ']') {
      CONFIG.STROKE_WEIGHT += 0.2;
      CONFIG.STROKE_WEIGHT = p.constrain(CONFIG.STROKE_WEIGHT, CONFIG.MIN_STROKE_WEIGHT, CONFIG.MAX_STROKE_WEIGHT);
      console.log(`Stroke weight increased to ${CONFIG.STROKE_WEIGHT.toFixed(1)}`);
    }

    // Decrease strokeWeight with '[' key
    if (p.key === '[') {
      CONFIG.STROKE_WEIGHT -= 0.2;
      CONFIG.STROKE_WEIGHT = p.constrain(CONFIG.STROKE_WEIGHT, CONFIG.MIN_STROKE_WEIGHT, CONFIG.MAX_STROKE_WEIGHT);
      console.log(`Stroke weight decreased to ${CONFIG.STROKE_WEIGHT.toFixed(1)}`);
    }
  };
}
