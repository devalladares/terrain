...
export default function sketch17(p) {
  ...
  const ISO_STEP = 20;                    // Step between iso-levels (e.g., 0, 40, 80, ...)
  ...
  const STROKE_WEIGHT_VAL = 0.5;            // Thickness of contour lines
  ...
  // -----------------------------------------------------------------------------
  p.preload = function() {
    // Load the image and assign it to 'img'
    img = p.loadImage(IMAGE_PATH, 
      () => {
        console.log(`Image "${IMAGE_PATH}" loaded successfully.`);
      },
      (err) => {
        console.error(`Failed to load image "${IMAGE_PATH}".`, err);
      }
    );
  };
