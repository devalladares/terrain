...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // If within effectRadius, add extra rotation each frame
      let effectRadius = 80; // increase or decrease for a bigger / smaller hover area
      if (d < effectRadius) {
        // Each frame, add some rotation to make the plus spin while hovered
        angle += 0.05;  // tweak this speed as you like (0.05 ~ 2.86 degrees/frame)
      }
      ...
      // Rotate using noise-based angle (plus any extra from hovering)
      rotate(v.heading());
