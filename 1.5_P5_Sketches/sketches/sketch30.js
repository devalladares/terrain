...
export default function(p) {
  ...
  p.setup = function() {
    ...
    p.textSize(10); // Adjust text size as needed
    p.colorMode(0);
  };
  
  p.draw = function() {
    p.background(0); // Inverted from black (0) to white (255)
    xoff = 0;
    for (let x = 0; x <= p.width; x += 16) {
      ...
      for (let y = 0; y <= p.height; y += 16) {
        ...
        if (distFromRipple <= rippleStrength) {
          ...
          p.fill(255); // Inverted from white (255) to black (0)
