...
export default function(p) {
  ...
  let inc = 0.1;   // Noise increment (smaller => smoother, more stretched flow)
  let scl = 200;     // Size of each flow-field cell (bigger => chunkier flow cells)
  let cols, rows;   // Number of columns and rows in the flow field
  let zoff = 10;     // Time offset for noise (creates animation)
  let flowField;    // Will hold a p5.Vector for each cell
  
  let particles = [];
  let numParticles = 300; // Fewer particles for more negative space / separation
  ...
  class Particle {
    ...
    show() {
      p.stroke(this.col);
      p.strokeWeight(10);  // Thicker stroke for a bolder, more graphic look
