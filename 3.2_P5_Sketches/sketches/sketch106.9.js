export default function(p) {
    ...
    const CONFIG = {
        ...
        colors: {
            ...
            terrain: {
                low:   [255, 255, 255, 120],  // Lower elevation color
                high:  [ 255, 255, 255, 180],  // Higher elevation color
    ...
    function drawTerrainWithBoids() {
        ...
        // 1) Draw the squares with a color influenced by boids
        for (let y = 0; y < CONFIG.rows - 1; y++) {
            for (let x = 0; x < CONFIG.cols - 1; x++) {
                ...
                // Check boid influence for this cell (top-left corner)
                const influence = getBoidInfluence(x, y);
                // Blend terrain color with boid color
                const boidColor = CONFIG.colors.boid; // [r,g,b]
                const finalColor = lerpColor(terrainColor, boidColor, influence);
