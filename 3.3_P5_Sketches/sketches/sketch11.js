...
export default function(p) {
    ...
    const GRID_COLS = 100;            // Number of cells horizontally
    const GRID_ROWS = 100;            // Number of cells vertically
    ...
    const STROKE_WEIGHT_INITIAL = 1;   // Initial stroke weight
    ...
    let zoom = INITIAL_ZOOM;         // Initial zoom level
    ...
    function setupGUI() {
      ...
      gui.add(params, 'strokeWeight', STROKE_WEIGHT_MIN, STROKE_WEIGHT_MAX).step(0.1).name('Stroke Weight').onChange(() => {
        // No additional action needed since strokeWeight is set in draw
      });
      ...
      // -----------------------------------------
      // 12. Auto-Rotate Toggle
      // -----------------------------------------
      gui.add({autoRotate: autoRotate}, 'autoRotate').name('Auto-Rotate').onChange((value) => {
        autoRotate = value;
      });
    }
