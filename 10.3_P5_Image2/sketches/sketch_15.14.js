export default function adaptedSketchN(p) {
  ...
  const controls = {
    ...
    // Animation controls
    flashDuration: 1000,    // Set to 1000ms as requested
  ...
  function updateFreeze() {
    ...
    if (timeSinceLastFreeze <= controls.flashDuration) {
      highlightedRow = currentRow;
               ...
               timeSinceLastFreeze <= controls.flashDuration + controls.rowFreezeDuration) {
  ...
  p.draw = () => {
    ...
    if (isAnimating) {
      updateFreeze();
      // Only update flow fields for unfrozen rows
      zOffset += zIncrement;
    ...
    for (let i = margin; i < N_COLS - margin; i++) {
      for (let j = margin; j < N_ROWS - margin; j++) {
        ...
        // Get the appropriate vector - either frozen or current
        let v;
        if (frozenCells.has(cellKey)) {
          v = frozenFlowField[cellKey].field;
        } else if (j === highlightedRow) {
          // For highlighted row, use the current field but don't animate
          v = field[i][j];
        } else {
          v = field[i][j];
        }
        ...
        // Use frozen letter if cell is frozen or in highlighted row
        let letter;
        if (frozenCells.has(cellKey)) {
          letter = frozenLetters[cellKey];
        } else if (j === highlightedRow) {
          // For highlighted row, freeze the current letter
  ...
  function updateFlowFields() {
    for (let i = 0; i < N_COLS; i++) {
      for (let j = 0; j < N_ROWS; j++) {
        ...
        const cellKey = `${i},${j}`;
        if (!frozenCells.has(cellKey) && j !== highlightedRow) {
