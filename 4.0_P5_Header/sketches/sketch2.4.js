export default function(p) {
  const CONFIG = {
    ...
    LINE_WEIGHT: 1,
    TRAIL_LENGTH: 200,  // Reduced to help with performance
    FLOW_VECTOR_SCALE: 3,
    ...
    PARTICLE_SETTINGS: {
      ...
      MAX_COUNT: Infinity,  // Changed to Infinity
  ...
  p.draw = function() {
    ...
    particles = particles.filter(particle => !isOutOfBounds(particle));
  ...
  function isOutOfBounds(particle) {
    const buffer = 100; // Allow particles to go slightly off-screen before considering them "out"
    return (
      particle.x < -buffer ||
  ...
  p.mouseDragged = function() {
    // Add new particles without limit
