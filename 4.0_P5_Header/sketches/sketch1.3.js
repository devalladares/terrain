...
export default function(p) {
  ...
  const CONFIG = {
    ...
    LINE_COLOR: {
      ...
      ALPHA: 255,                            // Alpha value for lines (optional)
    },
    ...
    FLOW_VECTOR_SCALE: 5,                  // Scale factor to increase movement per step

    // ===========================
    // Added Configuration for Trails and Mouse Seeding
    // ===========================
    TRAIL: {
      MAX_LENGTH: 5,       // Maximum number of points in trail history
      MIN_LENGTH: 10,       // Minimum trail length
      FADE_START: 0.8,      // When to start fading (percentage of max length)
    },
    MOUSE_SEED: {
      PARTICLES_PER_CLICK: 5,  // Number of particles to create per click
      SPREAD_RADIUS: 20        // Radius around click to spread new particles
    }
  };
  ...
  p.draw = function() {
    ...
    particles.forEach(particle => {
      ...
      if (particle.trail.length > CONFIG.TRAIL.MAX_LENGTH) {
      ...
      for (let i = 1; i < particle.trail.length; i++) {
        ...
        if (t < CONFIG.TRAIL.FADE_START) continue; // Skip fading until FADE_START

        const alpha = p.map(t, 
          CONFIG.TRAIL.FADE_START, 1, 
          CONFIG.LINE_COLOR.ALPHA, 0
        );
        
        p.stroke(CONFIG.LINE_COLOR.GRAY, alpha);
  ...
  /**
   * Creates a new particle with random position and initializes its trail.
   * @param {number} [x] - Optional X position.
   * @param {number} [y] - Optional Y position.
   * @returns {Object} Particle object with x, y, trail, and age properties.
   */
  function createParticle(x, y) {
    const startX = x !== undefined ? x : p.random(CONFIG.PARTICLE_RESET_POSITION.MIN_X, CONFIG.PARTICLE_RESET_POSITION.MAX_X());
    const startY = y !== undefined ? y : p.random(CONFIG.PARTICLE_RESET_POSITION.MIN_Y, CONFIG.PARTICLE_RESET_POSITION.MAX_Y());
    
    return {
      x: startX,
      y: startY,
      trail: [{x: startX, y: startY}], // Initialize trail with starting position
      age: 0
    };
  }

  /**
   * Resets a particle to a random position within specified bounds and clears its trail.
   * @param {Object} particle - The particle to reset.
   */
  function resetParticle(particle) {
    const newX = p.random(CONFIG.PARTICLE_RESET_POSITION.MIN_X, CONFIG.PARTICLE_RESET_POSITION.MAX_X());
    const newY = p.random(CONFIG.PARTICLE_RESET_POSITION.MIN_Y, CONFIG.PARTICLE_RESET_POSITION.MAX_Y());
    particle.x = newX;
    particle.y = newY;
    particle.trail = [{x: newX, y: newY}];
    particle.age = 0;
  }
  ...
  p.mousePressed = function() {
    // Only add particles if mouse is within canvas
    if (p.mouseX >= 0 && p.mouseX <= p.width && 
        p.mouseY >= 0 && p.mouseY <= p.height) {
      
      for (let i = 0; i < CONFIG.MOUSE_SEED.PARTICLES_PER_CLICK; i++) {
        // Create particles in a radius around the click
        const angle = p.random(p.TWO_PI);
        const r = p.random(CONFIG.MOUSE_SEED.SPREAD_RADIUS);
        const x = p.mouseX + r * p.cos(angle);
        const y = p.mouseY + r * p.sin(angle);
        
        particles.push(createParticle(x, y));
      }
      
      // Optional: Remove oldest particles if we have too many
      while (particles.length > CONFIG.NUM_PARTICLES * 1.5) {
        particles.shift();
      }

      console.log(`${CONFIG.MOUSE_SEED.PARTICLES_PER_CLICK} particles added at mouse position.`);
    }

    // Example: Reset all particles on mouse press (if desired)
    // Uncomment the following lines if you want to reset particles on every mouse press
    /*
