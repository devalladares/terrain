...
export default function(p) {
    ...
    const DRAW_STEPS_PER_FRAME = 300; 
    ...
    const TRAIL_LENGTH = 20; // You can adjust this value as needed
    
    // Directional bias vector to guide particles from bottom left to top right
    const BIAS_VECTOR_X = 0.1; // Horizontal bias component (rightward movement)
    const BIAS_VECTOR_Y = -0.1; // Vertical bias component (upward movement)
    ...
    p.setup = function() {
        ...
        // Initialize particles with positions near the bottom left and assign them colors from the current palette
        for (let i = 0; i < NUM_PARTICLES; i++) {
            particles.push({
                // Initialize x-coordinate near the left edge (with some randomness)
                x: p.random(0, p.width * 0.2),
                
                // Initialize y-coordinate near the bottom edge (with some randomness)
                y: p.random(p.height * 0.8, p.height),
    ...
    p.draw = function() {
        ...
        if (SHOW_BG_IMAGE) {
            ...
            p.noStroke();
            p.rect(0, 0, p.width, p.height, 10); // Semi-transparent rectangle to fade trails
        }
        ...
        for (let i = 0; i < DRAW_STEPS_PER_FRAME; i++) {
            ...
            // Apply a directional bias towards the top right
            let bias = p.createVector(BIAS_VECTOR_X, BIAS_VECTOR_Y); // Adjust these values to control the bias strength
            vx += bias.x;
            vy += bias.y;
