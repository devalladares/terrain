...
export default function(p) {
  ...
  // Noise Configuration
  const NOISE_SCALE_INITIAL = 0.05; // Initial noise scale (controls horizontal stretch)
  ...
  let params = {
    ...
    animate: true,
    darkMode: false,
    autorotate: true,              // Toggle for autorotation
    rotationSpeed: 0.001            // Speed of autorotation
  };

  // -----------------------------------------
  // 2. Global Variables
  // -----------------------------------------
  let currentPalette;               // Currently selected color palette
  let backgroundColor;              // Background color based on mode
  ...
  let zoom = 2;                     // Initial zoom level
  const ZOOM_FACTOR = 0.01;        // Zoom sensitivity

  // Rotation Angles
  let rotationX = -p.PI / 20;        // Rotate 200 degrees upwards
  let rotationY = p.PI /100;         // Rotate 45 degrees to the right
  let rotationZ = p.PI / 30;         // Apply a 45-degree anticlockwise tilt around Z-axis

  // Autorotation Variables
  let autorotate = params.autorotate;      // Flag to enable/disable autorotation
  let rotationSpeed = params.rotationSpeed; // Speed of autorotation
  ...
  p.draw = function() {
    ...
    // Apply Orbit Control without zoom (we handle zoom separately)
    p.orbitControl(1, 1, 0); // Disable default zoom by setting zoomFactor to 0

    // Apply initial rotations to face top right and apply anticlockwise tilt
    p.rotateX(rotationX);
    p.rotateY(rotationY);
    p.rotateZ(rotationZ);

    // Apply autorotation if enabled
    if (autorotate) {
      rotationY += rotationSpeed;
      rotationZ += rotationSpeed / 2; // Slightly different speed for variety
    }
  ...
  function setupGUI() {
    ...
    gui.add(params, 'heightScale', 100, 2000).step(10).name('Height Scale').onChange(() => {
      assignLevelColors();
    });

    // Number of Contour Levels Control
    gui.add(params, 'numLevels', 5, 50).step(1).name('Contour Levels').onChange(() => {
      assignLevelColors();
    });
    ...
    // Autorotate Toggle
    gui.add(params, 'autorotate').name('Autorotate').onChange((value) => {
      autorotate = value;
    });

    // Autorotate Speed Control
    gui.add(params, 'rotationSpeed', 0.0, 0.05).step(0.001).name('Rotation Speed').onChange((value) => {
      rotationSpeed = value;
    });

    // -----------------------------------------
    // 12. Custom Zoom Controls
    // -----------------------------------------
    // Add instructions or controls for zoom if desired
    // Note: We're handling zoom via mouseWheel separately
  }
