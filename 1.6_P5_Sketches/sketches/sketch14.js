...
export default function sketch13(p) {
  ...
  const margin = 2;              // Left/right/top/bot margin in "cell units"
  const gapFactor = 0.8;         // Gap between squares (as fraction of each cell's dimension)
  
  const palette = ["#003323", "white", "#96D39B", "#57D7F2", "#F098F4"];
  
  // Noise/animation parameters
  const offset = 10;             // Noise offset
  const rad = 2;                 // Used inside noise calls (radius amplitude)
